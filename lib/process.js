...
var types = {
    widget: function(item, index, appObj, success) {
        ...
        flow().seq(function(next){
            if (!checks.widgetChecks(item, index)) {
                console.log('\n');
                console.log('Your data.js file is showing some errors.'.bold.red);
                process.exit();
            } else {
                next();
            }
        }).seq(function(next){
            input.fetchWidget(item.source, function(data) {
                widgetData = data;
                next();
            }, function() {
                console.log('\n');
                console.log(('There was an error fetching the widget data for ' + item.source + '.').bold.red);
                process.exit();
            });
        }).seq(function(next){
            widgetData.tags = widgetData.tags.split(",");
            var splitName = widgetData.name.split(".");
            widgetData.name = _s.humanize(splitName[splitName.length - 1]);
    ...
    app: function(item, index, appObj, success) {
        if (!checks.standardChecks(item, index)) {
            console.log('\n');
            console.log('Your data.js file is showing some errors.'.bold.red);
            process.exit();
